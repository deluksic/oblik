...
- [13 — tracing a raster reference](./prototypes/13.md) — partly built. An `image(...)` trace node drawn as a textured quad, import by paste / drop / picker, and the transform kit: measure a known distance, rotate in 90° steps, flip, position, and style dials (opacity, saturation, contrast). The node, its patch endpoint, the pick arms, the GPU layer, and paste/drop import have landed; the inspector, the file picker, drag-to-move and measure are open.
