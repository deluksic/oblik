...
- [13 — tracing a raster reference](./prototypes/13.md) — partly built. An `image(...)` trace node drawn as a textured quad, import by paste / drop / picker, and the transform kit: measure a known distance, rotate in 90° steps, flip, position, and style dials (opacity, saturation, contrast). Built: the node and its patch endpoint, the pick arms, the GPU layer (quad, style dials, region-style selection chrome), import by paste / drop / picker, the inspector, and live previews that write once per gesture rather than once per pointer event. Open: drag-to-move and measure.
