...
App imports: `oblik` (eval/geom/source), `oblik/host` (`mountOblik`, scene picker), `virtual:oblik-catalog` + `scene-loaders.ts` in the demo. The Vite plugin **replaces** `scene-loaders.ts` on every transform with a disk-scanned `import()` map (plain JS — no `satisfies`). `vite.config.ts` must import the plugin `.ts` file with a relative path because Node loads the config.
