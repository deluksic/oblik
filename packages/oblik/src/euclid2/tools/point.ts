...
function applyLengthToAxis(
  session: PointSession,
  hit: PlaceHit,
  scope: Scope,
): PointSession | { insert: ReturnType<typeof withBind> } {
  ...
  if (session.focus === "x") {
    ...
    if (yExpr)
      return {
        insert: withBind(next, { from: "point", args: [{ kind: "ref", name: bind }, yExpr] }),
      };
    return next;
  }
  if (session.focus === "y") {
    ...
    if (xExpr)
      return {
        insert: withBind(next, { from: "point", args: [xExpr, { kind: "ref", name: bind }] }),
      };
    return next;
  }
  return session;
...
export const point: Tool<PointSession> = {
  ...
  click(session, hit, scope) {
    ...
    return {
      insert: withBind(session, { from: "point", args: pointArgs(session, hit.point.at, scope) }),
    };
  },
  ...
  preview(session, place, scope): Preview {
    ...
    if (
      p?.kind === "ref" &&
      !hasNumberBinding(session.x, scope) &&
      !hasNumberBinding(session.y, scope)
    ) {
