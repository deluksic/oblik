...
function placeFromVertex(
  ...
  const k =
    kArg && kArg.kind === "num" && (kArg.value === 1 || kArg.value === -1) ? kArg.value : undefined;
...
export const profile: Tool<ProfileSession> = {
  ...
  click(session, hit, scope) {
    ...
    const k =
      hit.carrier.geom.kind === "circle" ? hoverK(hit.carrier.geom, from.at, hit.world) : undefined;
  ...
  preview(session, place, scope): Preview {
    ...
    if (needCarrier(session) && place?.carrier) {
      const k =
        place.carrier.geom.kind === "circle" && from
          ? hoverK(place.carrier.geom, from.at, place.world)
          : undefined;
      extra.push(
        carrierExpr({
