...
/** User TypeScript under the demo app — not dts or node_modules. */
...
function walkTs(dir: string, out: string[]): void {
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    if (name.endsWith(".ts") && !name.endsWith(".d.ts")) out.push(abs);
  }
}

/** Vite import specifiers relative to `appRoot/src`. */
