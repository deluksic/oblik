...
export function oblikVanillaExtractHmr(): Plugin {
  return {
    ...
    async hotUpdate({ file, server, timestamp }) {
      ...
      // The generated CSS lives in the compiler's cache, refreshed only when
      // the .css.ts is re-transformed. Re-run it here so the cache is fresh
      // before the browser fetches the virtual module; otherwise the update
      // races the compiler's own watcher and can serve stale CSS.
      for (const mod of graph.getModulesByFile(fileId) ?? []) {
        try {
          await server.environments.client.transformRequest(mod.url);
        } catch {
          // A broken .css.ts will surface when the browser fetches it.
        }
      }
