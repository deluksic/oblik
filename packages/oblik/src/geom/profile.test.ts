...
describe("profile holes", () => {
  test("a hole punches the interior and is not XOR", () => {
    const p = regionValue(rectCycle(0, 0, 1, 1), { holes: [rectCycle(0.25, 0.25, 0.75, 0.75)] });
  ...
  test("an arc hole keeps A commands in the svg path", () => {
    ...
    const P = { x: 0.3, y: 0.5 };
    const Q = { x: 0.7, y: 0.5 };
    const p = regionValue(rectCycle(0, 0, 1, 1), {
  ...
  test("a circle is a closed walk as a hole", () => {
    ...
    const p = regionValue(rectCycle(0, 0, 1, 1), { holes: [hole] });
    expect(p.holes).toHaveLength(1);
  ...
  test("a hole outside the outer is an empty profile", () => {
    const p = regionValue(rectCycle(0, 0, 1, 1), { holes: [rectCycle(2, 2, 3, 3)] });
    expect(p.outer).toHaveLength(0);
  ...
  test("a hole that crosses the outer is empty", () => {
    const p = regionValue(rectCycle(0, 0, 1, 1), { holes: [rectCycle(0.5, 0.5, 1.5, 1.5)] });
    expect(p.outer).toHaveLength(0);
  });

  test("roundOffset of a holed profile shrinks the hole when growing", () => {
    const p = regionValue(rectCycle(0, 0, 1, 1), { holes: [rectCycle(0.3, 0.3, 0.7, 0.7)] });
