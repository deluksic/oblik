...
- [?scene=rose](http://127.0.0.1:43117/?scene=rose) — **joined rings, then quatrefoil cuts.** Three panes: packed plan (`cylinder.ts` gizmos, no cylinder solid), sweep profile (`profile.ts`, X radial / Y is Z), SDF field. Height is the 3D glider (cut height). `cylinder()` stays in the sdf library.
