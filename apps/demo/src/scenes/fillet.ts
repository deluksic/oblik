...
export default defineScene({
  kind: "euclid2",
  title: "Fillet",
  hint: "Every fillet(·, r) shares r. Drag r until the small square vanishes (adjacent overlap). Below r = 0.12 the inner offset is a sharp square (eaten fillet arcs). The semicircle keeps its diameter (fillet of a flat origin is a no-op).",
  ...
  build() {
    ...
    const rim = region(
      [Ro, roa, fillet(Rp, r), along(Rreach, 1), fillet(Rq, r), rob],
      [],
      "o_fil_rim",
    );
