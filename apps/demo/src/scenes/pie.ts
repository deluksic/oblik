...
export default defineScene({
  ...
  build() {
    ...
    const one = roundOffset(
      region([O, oa, A, along(reach, 1), B, ob], [], "o_2677e005a4"),
      -gap,
      "o_pie_1",
    );
    const two = roundOffset(
      region([O, ob, B, along(reach, 1), C, oc], [], "o_6c78f3e66f"),
      -gap,
      "o_pie_2",
    );
    const three = roundOffset(
      region([O, oc, C, along(reach, 1), A, oa], [], "o_9b79258711"),
      -gap,
      "o_pie_3",
    );
